...
contract MerkleDistributor is IMerkleDistributor, Ownable {
    ...
    constructor(address token_, bytes32 merkleRoot_, uint256 perUserClaimLimit_) {
        token = token_;
    ...
    function claim(uint256 index, bytes32[] calldata merkleProof)
    public
    virtual
    override
    {
...
// File contracts/distributor/MerkleDistributorWithDeadline.sol
...
contract MerkleDistributorWithDeadline is MerkleDistributor {
    ...
    constructor(address token_, bytes32 merkleRoot_, uint256 perUserClaimLimit_,uint256 endTime_) MerkleDistributor(token_, merkleRoot_,perUserClaimLimit_) {
        if (endTime_ <= block.timestamp) revert EndTimeInPast();
        endTime = endTime_;
    }

    function claim(uint256 index, bytes32[] calldata merkleProof) public override {
        if (block.timestamp > endTime) revert ClaimWindowFinished();
        super.claim(index, merkleProof);
    }
